...
const apiKey = process.env.VOTTUN_API_KEY || process.env.SMITHERY_API_KEY || "";
      ...
      if (!apiKey) {
        return {
          content: [
            {
              type: "text",
              text: "Missing VOTTUN_API_KEY (or SMITHERY_API_KEY). get_certificate requires API key auth (X-API-Key)."
            }
          ],
